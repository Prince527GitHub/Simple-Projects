...
async function getPipedUrl() {
    let item = await browser.storage.sync.get("piped");
    return item.piped;
}
...
async function sendToMeTube(itemUrl, quality, format) {
    let pipedUrl = await getPipedUrl();

    itemUrl = itemUrl || await getCurrentUrl();
    if (pipedUrl && itemUrl.includes(pipedUrl)) itemUrl = itemUrl.replace(pipedUrl, "youtube.com");
